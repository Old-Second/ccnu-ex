...
const 计分 = _ => {
    ...
    分数[肉] = `得分: ${分}`;
    称号1[肉] = `${分 > 100 ? "华" : 分 > 80 ? "桂" : 分 > 60 ? "登" : 分 > 40 ? "小" : 分 > 20 ? "初" : "足"}`;
    称号2[肉] = `${分 > 100 ? "师" : 分 > 80 ? "子" : 分 > 60 ? "山" : 分 > 40 ? "有" : 分 > 20 ? "入" : "不"}`;
    称号3[肉] = `${分 > 100 ? "万" : 分 > 80 ? "山" : 分 > 60 ? "强" : 分 > 40 ? "探" : 分 > 20 ? "华" : "出"}`;
    称号4[肉] = `${分 > 100 ? "事" : 分 > 80 ? "庄" : 分 > 60 ? "者" : 分 > 40 ? "索" : 分 > 20 ? "师" : "户"}`;
    称号5[肉] = `${分 > 100 ? "通" : 分 > 80 ? "主" : 分 > 60 ? "" : 分 > 40 ? "" : 分 > 20 ? "" : ""}`;
}
const 统计进度 = _ => {
    const 进度数 = 获取所有省等级们().reduce((全, 当前) => {
        return 全 + (+当前 ? 1 : 零);
    }, 零);
    进度[肉] = `探索进度: ${进度数}/49`;
}
...
const 获取字体样式 = (字体名, 回调) => {
    获取字体数据地址(`${字体名}.woff?v={version}`, 地址 => 回调(`@font-face{font-family:${字体名};${源}:url(${地址})}`));
};
获取字体样式(字体名, 样式字串 => {
    $(样式, 图形)[肉] = 样式字串;
    const 样式元素 = 新建元素(样式);
...
const 获取字体样式1 = (字体名, 回调) => {
    获取字体数据地址(`${字体名}.ttf?v={version}`, 地址 => 回调(`@font-face{font-family:${字体名};${源}:url(${地址})}`));
};
获取字体样式1('font1', 样式字串 => {
    $(样式, 图形)[肉] = 样式字串;
    const 样式元素 = 新建元素(样式);
...
const 保存图像 = _ => {
    ...
    地址变图像元素(数据地址, 图 => {
        ...
        上下文.drawImage(图, 零, 零, 宽, 高, 零, 零, 宽 * 比, 高 * 比);
        画板.toBlob(元素数据 => {
